...
def test_henonmap(test_context):
    ...
    px_all_n = (
        alpha_x * x_all / np.sqrt(beta_x) * lmbd + px_all * np.sqrt(beta_x) * lmbd
    )
    ...
    x_out_test = cos_omega_x * x_in_test + sin_omega_x * (
        px_in_test + (x_in_test**2 - y_in_test**2)
    )
    px_out_test = -sin_omega_x * x_in_test + cos_omega_x * (
        px_in_test + (x_in_test**2 - y_in_test**2)
    )
    y_out_test = cos_omega_y * y_in_test + sin_omega_y * (
        py_in_test - 2 * x_in_test * y_in_test
    )
    py_out_test = -sin_omega_y * y_in_test + cos_omega_y * (
        py_in_test - 2 * x_in_test * y_in_test
    )

    p_test = xp.Particles(
        x=x_in_test, px=px_in_test, y=y_in_test, py=py_in_test, _context=test_context
    )
    ...
    x_out = test_context.nparray_from_context_array(p_test.x)[
        np.argsort(test_context.nparray_from_context_array(p_test.particle_id))
    ]
    px_out = test_context.nparray_from_context_array(p_test.px)[
        np.argsort(test_context.nparray_from_context_array(p_test.particle_id))
    ]
    y_out = test_context.nparray_from_context_array(p_test.y)[
        np.argsort(test_context.nparray_from_context_array(p_test.particle_id))
    ]
    py_out = test_context.nparray_from_context_array(p_test.py)[
        np.argsort(test_context.nparray_from_context_array(p_test.particle_id))
    ]
    ...
    p_inv_test = xp.Particles(
        x=x_out, px=px_out, y=y_out, py=py_out, _context=test_context
    )

    line_test.track(p_inv_test, backtrack=True)

    x_in_test_inv = test_context.nparray_from_context_array(p_inv_test.x)[
        np.argsort(test_context.nparray_from_context_array(p_inv_test.particle_id))
    ]
    px_in_test_inv = test_context.nparray_from_context_array(p_inv_test.px)[
        np.argsort(test_context.nparray_from_context_array(p_inv_test.particle_id))
    ]
    y_in_test_inv = test_context.nparray_from_context_array(p_inv_test.y)[
        np.argsort(test_context.nparray_from_context_array(p_inv_test.particle_id))
    ]
    py_in_test_inv = test_context.nparray_from_context_array(p_inv_test.py)[
        np.argsort(test_context.nparray_from_context_array(p_inv_test.particle_id))
    ]
