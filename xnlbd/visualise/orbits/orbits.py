...
def _get_H_orbit_points(
    ...
    x_test = np.linspace(part_on_co.x[0], part_on_co.x[0] + line.config.XTRACK_GLOBAL_XY_LIMIT, num_pts)
    part.x = x_test
...
def _get_V_orbit_points(
    ...
    """
    # Find rough estimate for limit of stability
    y_test = np.linspace(part_on_co.y[0], part_on_co.y[0] + line.config.XTRACK_GLOBAL_XY_LIMIT, num_pts)
    part.y = y_test
    part_copy = copy.deepcopy(part)
    line.track(
        part,
